...
const Proyects = () => {
  ...
  const [proyectos, setProyectos] = useState<IProyect[]|null>(null)

  useEffect(()=>{
    const fetchData = async()=>{
      const proyects: IProyect[] = await getProyects()
                  ...
                  <div className='bg-blue-100 flex justify-end py-2 relative'>
                    {
                      proyect.skills && proyect.skills.map((skill)=>{
                        return(
                          <div className='group' key={skill._id}>
                            <img 
                              src={skill.image} 
                              alt={`skill${skill._id}`}                               
                    ...
                    <div className='bg-white w-1/2 opacity-100 flex flex-col items-center relative'>
                      {
                        ...
                        <>
                          <div className='p-4 text-3xl text-white bg-primaryColor w-full text-center'>
                            ...
                            {
                              modalProyect.url ? (<Link className='text-primaryColor my-4 text-2xl transition duration-300 ease-in-out cursor-pointer hover:scale-105' 
                              href={modalProyect.url} target="_blank" >Visitar</Link>):(<div className='mt-8'></div>)
                            }
                          ...
                          <div className='grid grid-cols-6 gap-6 my-8'>
                          {
                            modalProyect.skills && modalProyect.skills.map((skill)=>{
                              return(
                                <div className='group flex flex-col justify-center items-center' key={skill._id}>
                                  <img 
                                    src={skill.image} 
                                    alt={`skill${skill._id}`}                               
